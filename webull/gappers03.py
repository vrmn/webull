...
def gappers(stocks,num):
    ...
    hqm_columns = ['symbol','lastPrice','netChange','totalVolume','volatility','marketcap','highPrice']
    hqm_dataframe = pd.DataFrame(columns=hqm_columns)
    for symbol_string in symbol_strings:
        url = 'https://api.tdameritrade.com/v1/marketdata/quotes'
        url2 = 'https://api.tdameritrade.com/v1/instruments'
        payload = {'apikey':'FCLFLNW0R2IBBC9NPIRMIRR6WDCFSS2J',
                    'symbol':symbol_string}
        # print(requests.get(url,params=payload))
        data = requests.get(url,params=payload).json()
        data2 = requests.get(url2,params=payload).json()
    ...
    # FILTER 
    hqm_dataframe = hqm_dataframe[(hqm_dataframe['netChange']> 3)&(hqm_dataframe['lastPrice']<=15)]
